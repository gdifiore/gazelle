...
static void test_empty_after_init(void)
{
    ...
    char buf[32];
    uint8_t len;
    ASSERT(!ipc_read(buf, sizeof(buf), &len));
}

static void test_write_read_roundtrip(void)
{
    ipc_init();
    ASSERT(ipc_write("hello", 6)); /* len includes the null terminator */
    char buf[32];
    uint8_t len;
    ASSERT(ipc_read(buf, sizeof(buf), &len));
...
static void test_write_full_fails(void)
{
    ...
    for (int i = 0; i < IPC_BUFFER_SIZE; i++)
        ASSERT(ipc_write("x", 2));
    ASSERT(!ipc_write("x", 2)); /* one beyond capacity */
}
...
static void test_write_len_at_limit_fails(void)
{
    ...
    ipc_init();
    ASSERT(!ipc_write("x", IPC_DATA_SIZE));
}
...
static void test_failed_read_does_not_consume_message(void)
{
    ...
    char buf[4];
    uint8_t len;
    ASSERT(!ipc_read(buf, 4, &len)); /* fails */

    char buf2[32];
    ASSERT(ipc_read(buf2, sizeof(buf2), &len)); /* message still there */
...
static void test_wraparound(void)
{
    ...
    for (int i = 0; i < IPC_BUFFER_SIZE; i++)
        ASSERT(ipc_write("wrap", 5));

    ASSERT(!ipc_write("x", 2)); /* should be full */
